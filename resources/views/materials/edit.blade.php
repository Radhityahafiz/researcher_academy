        ...
        <div class="col-lg-12">
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex justify-content-between align-items-center">
                    <h6 class="m-0 font-weight-bold text-primary">Edit Materi: {{ $material->title }}</h6>
                </div>
                <div class="card-body">
                    @if($errors->any())
                    ...
                    <form action="{{ route('materials.update', $material) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control @error('content_type') is-invalid @enderror" id="content_type" name="content_type" required>
                                ...
                                <option value="text" {{ old('content_type', $material->content_type) == 'text' ? 'selected' : '' }}>Teks</option>
                            </select>
                            @error('content_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="form-group" id="file_upload_field" style="display: {{ old('content_type', $material->content_type) == 'file' ? 'block' : 'none' }};">
                            <label for="file">Upload File</label>
                            @if($material->file_path)
                                <div class="mb-2">
                                    <span class="badge bg-info">File saat ini: {{ basename($material->file_path) }}</span>
                                    <a href="{{ Storage::url($material->file_path) }}" target="_blank" class="btn btn-sm btn-outline-primary">Lihat File</a>
                                </div>
                            @endif
                        ...
                        <div class="form-group" id="external_link_field" style="display: {{ old('content_type', $material->content_type) == 'link' ? 'block' : 'none' }};">
                        ...
                        <div class="form-group" id="text_content_field" style="display: {{ old('content_type', $material->content_type) == 'text' ? 'block' : 'none' }};">
